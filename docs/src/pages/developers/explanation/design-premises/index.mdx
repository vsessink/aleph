...
### Entity linkages
Entities in one collection cannot link to entities in another. This is done as a security measure at the moment, but it's possible that other mitigations could be found.

### Collections for users, groups
**Users and groups are not collections**. It's possible we might automatically create a collection to match each role, but this means making them do weird things \(they cannot be shared?\)
